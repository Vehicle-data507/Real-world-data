...
# 设置模型为评估模式
# 获取训练集的真实值和预测值
train_true = y_train[:, -length_size:].detach().cpu()  # 获取训练集真实值
train_pred = trained_model(x_train.to(device), x_train_mark.to(device), y_train.to(device), y_train_mark.to(device)).detach().cpu()  # 获取训练集预测值

# 获取测试集的真实值和预测值
test_true = y_test[:, -length_size:].detach().cpu()  # 获取测试集真实值
test_pred = trained_model(x_test.to(device), x_test_mark.to(device), y_test.to(device), y_test_mark.to(device)).detach().cpu()  # 获取测试集预测值

# 检查 train_true 和 train_pred 的维度
print("Shape of train_true:", train_true.shape)
print("Shape of train_pred:", train_pred.shape)

# 假设 train_true 和 train_pred 是三维的，我们需要选择最后一个特征
train_true = train_true[:, 0, -1].flatten()  # 获取训练集最后一个时间步的真实值
train_pred = train_pred[:, 0, -1].flatten()  # 获取训练集最后一个时间步的预测值

# 处理测试集的维度
test_true = test_true[:, 0, -1].flatten()  # 获取测试集最后一个时间步的真实值
test_pred = test_pred[:, 0, -1].flatten()  # 获取测试集最后一个时间步的预测值

# 逆归一化
train_true_uninverse = scaler.inverse_transform(train_true.reshape(-1, 1))
train_pred_uninverse = scaler.inverse_transform(train_pred.reshape(-1, 1))

test_true_uninverse = scaler.inverse_transform(test_true.reshape(-1, 1))
test_pred_uninverse = scaler.inverse_transform(test_pred.reshape(-1, 1))

# 合并训练集和测试集的预测结果和真实值
true_all = np.concatenate([train_true_uninverse, test_true_uninverse], axis=0)
pred_all = np.concatenate([train_pred_uninverse, test_pred_uninverse], axis=0)

# 确保合并后的长度一致
print(f"Length of true_all: {len(true_all)}")
print(f"Length of pred_all: {len(pred_all)}")

# 将整个数据集的预测结果与真实值保存为一个 DataFrame
result_all_df = pd.DataFrame({'真实值': true_all.flatten(), '预测值': pred_all.flatten()})

# 保存整个数据集的结果到 CSV 文件
result_all_df.to_csv('整个数据集真实值与预测值-TCN+MHDA+iTransformer.csv', index=False, encoding='utf-8')
print('整个数据集的真实值与预测值已保存到整个数据集真实值与预测值.csv文件中。')

# 生成整个数据集的对比图（训练集和测试集的对比）
plt.figure(figsize=(12, 6))
plt.plot(range(len(train_true_uninverse)), train_true_uninverse.flatten(), label='True Train', color='blue', alpha=0.6)
plt.plot(range(len(train_pred_uninverse)), train_pred_uninverse.flatten(), label='Predicted Train', color='green', alpha=0.6)
plt.plot(range(len(train_true_uninverse), len(train_true_uninverse) + len(test_true_uninverse)), test_true_uninverse.flatten(), label='True Test', color='orange', alpha=0.6)
plt.plot(range(len(train_true_uninverse), len(train_true_uninverse) + len(test_pred_uninverse)), test_pred_uninverse.flatten(), label='Predicted Test', color='red', alpha=0.6)
...
plt.title('True vs Predicted Values (Train and Test) - Entire Dataset', fontsize=14, fontweight='bold')
